			...
			for (i = 0; i < Array_GetSize(&g_enemies); i++)
			{
				...
				if (GameObject_IsAlive(enemy))
				{
					if (GameObject_IsHit(enemy, &g_player))
					{
						...
						DrawFormatString((int) GameObject_GetX(&g_field, LEFT, -10), (int) GameObject_GetY(&g_field, TOP, -(i + 1) * 10.f), COLOR_WHITE, "当たっている");
					}
					else
					{
						DrawFormatString((int) GameObject_GetX(&g_field, LEFT, -10), (int) GameObject_GetY(&g_field, TOP, -(i + 1) * 10.f), COLOR_WHITE, "当たっていない");
					}
				}
			}
		}
